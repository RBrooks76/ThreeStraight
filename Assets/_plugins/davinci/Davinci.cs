...
public class Davinci : MonoBehaviour
{
    ...
    /// Set target Image component.
    /// </summary>
    /// <param name="rawImage">target Unity UI image component</param>
    /// <returns></returns>
        ...
        if (targetObj != null)
            switch (rendererType)
            {
                ...
                case RendererType.rawImage:
                    ...
                    if (fadeTime > 0 && rawimage.material.HasProperty("_Color"))
                    {
                        color = rawimage.material.color;
                        maxAlpha = color.a;
    ...
    private void finish()
    {
        ...
        if (!cached)
        {
            try
            {
                Debug.Log("Cache false!  Deleting Files ====>");
